...
type Address = {
  ...
  email?: string;
  phone: string;
  address: string; // Stores plain formatted address (legacy JSON still supported in parser)
  is_default: boolean;
  created_at?: string;
};
...
export default function ChangeAddress({ onSelectAddress }: ChangeAddressProps) {
  ...
  const parseAddressPayload = (raw: any) => {
    ...
    // 1) Try JSON payload first (legacy mobile format)
    try {
      const parsed = JSON.parse(rawText);
      if (parsed && typeof parsed === 'object') {
        parsedLabel = String(parsed.label || '').trim();
        parsedStreet = String(parsed.street || parsed.addressLine1 || parsed.line1 || '').trim();
        parsedLine2 = String(parsed.addressLine2 || parsed.line2 || '').trim();
        parsedCity = String(parsed.city || parsed.town || parsed.municipality || '').trim();
        parsedState = String(parsed.stateRegion || parsed.state || parsed.province || '').trim();
        parsedZip = String(parsed.zipCode || parsed.postalCode || parsed.zip || '').trim();
        display = String(
          parsed.display ||
          [parsedStreet, parsedLine2, parsedCity, parsedState, parsedZip].filter(Boolean).join(', ')
        ).trim();
      }
    } catch {
      // non-JSON payload, continue below
    }

    // 2) Plain string fallback (website-like format)
    if (!parsedStreet && !parsedCity && !parsedState && !parsedZip) {
      const cleaned = rawText.replace(/^Address:\s*/i, '').trim();
      const segments = cleaned.split(',').map((s) => s.trim()).filter(Boolean);

      if (segments.length >= 4) {
        parsedZip = segments[segments.length - 1] || '';
        parsedState = segments[segments.length - 2] || '';
        parsedCity = segments[segments.length - 3] || '';
        const head = segments.slice(0, Math.max(1, segments.length - 3));
        parsedStreet = head[0] || '';
        parsedLine2 = head.slice(1).join(', ');
      } else {
        parsedStreet = cleaned;
      }
  ...
  const openEditForm = (addr: Address) => {
    ...
    const parsedAddress = parseAddressPayload(addr.address);

    setLabel(addr.label || parsedAddress.label || "");
    setStreet(parsedAddress.street || "");
    setAddressLine2(parsedAddress.addressLine2 || "");
    setCity(parsedAddress.city || "");
    setStateRegion(parsedAddress.stateRegion || "");
  ...
  const performSave = async (user: any, makeDefault: boolean) => {
    const combinedPlain = [street, addressLine2, city, stateRegion, zipCode]
      .map((v) => String(v || '').trim())
      .filter(Boolean)
      .join(', ');
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: darkMode ? '#101010' : '#f5f5f5' }}>
      {/* Header */}
      <View style={{
        ...
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
          ...
          {loading ? (
            ...
            <View>
              {addresses.map((addr) => {
                const parsedAddress = parseAddressPayload(addr.address);
                const displayAddress = parsedAddress.display || addr.address;
                const displayLabel = String(addr.label || parsedAddress.label || '').trim();
                      ...
                      {displayLabel ? (
                          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
                            <View style={{ backgroundColor: '#8B1C1C', borderRadius: 4, paddingHorizontal: 8, paddingVertical: 2 }}>
                              <Text style={{ color: '#fff', fontSize: 11, fontWeight: '700' }}>{displayLabel}</Text>
                            </View>
                          </View>
                      ) : null}

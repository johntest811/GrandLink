 ...
 * - All cart items are stored in the 'cart' table in Supabase (same as web)
 * - The cart table has: id, user_id, product_id, quantity, meta, created_at, updated_at
 ...
 * - Both web and mobile now use the same 'cart' table for perfect synchronization
 */

import React, { useState, useEffect } from 'react';
import { View, Text, Image, TouchableOpacity, StyleSheet, FlatList, ScrollView, RefreshControl, TextInput } from 'react-native';
...
export default function CartScreen() {
  ...
  const loadCart = async (isRefreshing = false) => {
    try {
      ...
      // Use the same 'cart' table as the web version for synchronization
      ...
      setQuantityInputs(
        Object.fromEntries(
          items.map((item: any) => [item.id, String(item.qty || 1)])
        )
      );
  ...
  const removeFromCartDirect = async (id: string) => {
  ...
  const updateQuantity = async (id: string, newQty: number) => {
    if (newQty < 1) {
      removeFromCartDirect(id).catch((e) => {
        console.error('Failed to remove item at zero quantity', e);
        modal.showError('Error', 'Failed to remove item.');
      });
      return;
    }
    ...
    const stockLimit = Number(previousItem?.stock);
    const hasStockLimit = Number.isFinite(stockLimit) && stockLimit > 0;
    const cappedQty = hasStockLimit ? Math.min(newQty, stockLimit) : newQty;

    if (hasStockLimit && newQty > stockLimit) {
      modal.showWarning('Stock Limit', `Only ${stockLimit} unit${stockLimit === 1 ? '' : 's'} available for this item.`);
    }

    if (previousItem) {
      setCartItems(prev =>
        prev.map(item => (item.id === id ? { ...item, qty: cappedQty } : item))
      );
      setQuantityInputs(prev => ({ ...prev, [id]: String(cappedQty) }));
    }

    try {
      // Use cart table structure to match web version
      ...
      if (previousItem) {
        ...
        setQuantityInputs(prev => ({ ...prev, [id]: String(previousItem.qty ?? 1) }));
      }
  ...
  const onQuantityInputChange = (id: string, value: string) => {
    const numericOnly = value.replace(/[^0-9]/g, '');
    setQuantityInputs(prev => ({ ...prev, [id]: numericOnly }));
  };

  const commitQuantityInput = (id: string) => {
    const currentItem = cartItems.find(item => item.id === id);
    const currentQty = currentItem?.qty ?? 1;
    const stockLimit = Number(currentItem?.stock);
    const hasStockLimit = Number.isFinite(stockLimit) && stockLimit > 0;
    const inputValue = quantityInputs[id];

    if (!inputValue) {
      setQuantityInputs(prev => ({ ...prev, [id]: String(currentQty) }));
      return;
    }

    const parsed = parseInt(inputValue, 10);
    let nextQty = Number.isFinite(parsed) ? Math.max(1, parsed) : currentQty;
    if (hasStockLimit) {
      nextQty = Math.min(nextQty, stockLimit);
      if (Number.isFinite(parsed) && parsed > stockLimit) {
        modal.showWarning('Stock Limit', `Only ${stockLimit} unit${stockLimit === 1 ? '' : 's'} available for this item.`);
      }
    }

    setQuantityInputs(prev => ({ ...prev, [id]: String(nextQty) }));
  ...
  const sectionBg = darkMode ? '#1a1a1a' : '#fff';
  const subtleBg = darkMode ? '#202020' : '#f9f9f9';
  const border = darkMode ? '#2f2f2f' : '#eee';
    ...
    <View style={[styles.container, { backgroundColor: pageBg }]}>
      ...
      {loading ? (
        <View style={styles.centerContent}>
          <Text style={{ color: textSecondary }}>Loading cart...</Text>
        </View>
      ) : cartItems.length === 0 ? (
        <View style={styles.centerContent}>
          <Ionicons name="cart-outline" size={80} color={darkMode ? '#6a6a6a' : '#ccc'} />
          <Text style={[styles.emptyText, { color: textSecondary }]}>Your cart is empty</Text>
        ...
        <>
          {/* Select All Header */}
          <View style={[styles.selectAllContainer, { backgroundColor: subtleBg, borderBottomColor: border }]}>
            <TouchableOpacity
              style={styles.selectAllRow}
              onPress={toggleSelectAll}
            >
              <View style={[styles.checkbox, { backgroundColor: sectionBg, borderColor: darkMode ? '#4a4a4a' : '#ddd' }]}>
                {selectedItems.size === cartItems.length && (
                  <Ionicons name="checkmark" size={18} color="#fff" />
                )}
              </View>
              <Text style={[styles.selectAllText, { color: textPrimary }]}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={[styles.cartItem, { backgroundColor: sectionBg, borderColor: border }] }>
                <TouchableOpacity
                  style={styles.checkboxContainer}
                  onPress={() => toggleItemSelection(item.id)}
                >
                  <View style={[styles.checkbox, { backgroundColor: sectionBg, borderColor: darkMode ? '#4a4a4a' : '#ddd' }, selectedItems.has(item.id) && styles.checkboxSelected]}>
                ...
                {item.image ? (
                  ...
                  <View style={[styles.cartItemImagePlaceholder, { backgroundColor: darkMode ? '#242424' : '#f5f5f5' }]}>
                    <Ionicons name="image-outline" size={30} color={darkMode ? '#717171' : '#ccc'} />
                  </View>
                )}
                ...
                <View style={styles.itemDetails}>
                  ...
                  <View style={styles.quantityRow}>
                    <TouchableOpacity
                      style={[styles.qtyButton, { backgroundColor: darkMode ? '#262626' : '#f5f5f5', borderColor: darkMode ? '#4a4a4a' : '#ddd' }]}
                    ...
                    <TextInput
                      style={[
                        ...
                        {
                          color: textPrimary,
                          borderColor: darkMode ? '#4a4a4a' : '#ddd',
                          backgroundColor: darkMode ? '#181818' : '#fff',
                        },
                      ]}
                      value={quantityInputs[item.id] ?? String(item.qty ?? 1)}
                      onChangeText={(value) => onQuantityInputChange(item.id, value)}
                      onBlur={() => commitQuantityInput(item.id)}
                      onSubmitEditing={() => commitQuantityInput(item.id)}
          ...
          <View style={[styles.footer, { backgroundColor: sectionBg, borderTopColor: border }]}>
            <View style={styles.totalRow}>
              <View>
                <Text style={[styles.totalLabel, { color: textPrimary }]}>
                  Total ({selectedItems.size} {selectedItems.size === 1 ? 'item' : 'items'}):
                </Text>
                {selectedItems.size < cartItems.length && (
                  <Text style={[styles.subtotalText, { color: textSecondary }]}>Cart total: ₱{cartTotal.toFixed(2)}</Text>
                )}
              </View>
      ...
      <View style={styles.bottomNavBar}>
        ...
        <TouchableOpacity style={styles.navItem} onPress={() => router.push('../inquire')}>
...
const styles = StyleSheet.create({
  ...
  bottomNavBar: {
    ...
    backgroundColor: '#1a1a1a', // Black theme to match app
    height: 70,
    paddingBottom: 8,
    paddingTop: 8,
  ...
  fabButton: {
    ...
    shadowRadius: 6,
    borderWidth: 3,
    borderColor: '#1a1a1a', // Match black theme
  },
  fabIcon: {
    width: 32,
    height: 32,

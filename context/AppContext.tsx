...
const DARK_MODE_STORAGE_KEY = 'app_dark_mode_enabled_v1';
...
export function AppContextProvider({ children }: { children: ReactNode }) {
  ...
  const isInvalidRefreshTokenError = (error: unknown) => {
    ...
    return (
      ...
      (normalized.includes('refresh token') && normalized.includes('not found'))
    );
  };
  ...
  useEffect(() => {
    const loadDarkModePreference = async () => {
      try {
        const stored = await AsyncStorage.getItem(DARK_MODE_STORAGE_KEY);
        if (stored !== null) {
  ...
  const updateDarkMode = (value: boolean) => {
    setDarkMode(value);
    AsyncStorage.setItem(DARK_MODE_STORAGE_KEY, value ? 'true' : 'false').catch(() => {});
  };
  ...
  useEffect(() => {
    ...
    const startAuthRefresh = async () => {
      try {
        ...
        if (isMounted) {
          console.warn('Failed to start mobile auth refresh:', error);
  ...
  useEffect(() => {
    ...
    const initializeNotifications = async () => {
      ...
      try {
        ...
        console.warn('Failed to load current user for notifications:', error);
      }
    };
  ...
  const value = useMemo(() => ({ darkMode, setDarkMode: updateDarkMode }), [darkMode]);
